...
function initializeTables() {
    const count = Number(localStorage.getItem(TABLE_COUNT_KEY)) || 12;
...
function getSelectedTable() {
    return Number(localStorage.getItem("barSelectedTable") || 0);
}

function selectTable(id) {
    localStorage.setItem("barSelectedTable", String(id));
    renderTables();
    renderOrder();
}
...
function initializeInventory() {
    const products = getJSON(PRODUCTS_KEY, DEFAULT_PRODUCTS);
    let stock = getJSON(INVENTORY_KEY, {});
...
function getStock(productId) {
    const stock = getJSON(INVENTORY_KEY, {});
    return Number(stock[productId] ?? 0);
}

function changeStock(productId, amount) {
    const stock = getJSON(INVENTORY_KEY, {});
    stock[productId] = Math.max(0, Number(stock[productId] || 0) + amount);
    setJSON(INVENTORY_KEY, stock);
}
...
function updateClock() {
    ...
    clock.textContent = now.toLocaleTimeString("sq-AL", {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit"
    });
...
function renderHeader() {
    ...
    if (appName) {
        appName.textContent =
            localStorage.getItem(APP_NAME_KEY) || "MY BAR";
    }

    if (userInfo && user) {
        userInfo.textContent =
            user.name + " • " +
            (user.role === "admin" ? "ADMIN" : "KAMARIER");
    }
}
...
function renderTables() {
    ...
    const html = tables.map(table => {
        const active = table.items && table.items.length > 0;
        ...
        return `
            <button
                class="table-card ${active ? "active" : ""} ${selected === table.id ? "selected" : ""}"
                onclick="selectTable(${table.id})"
            >
                <strong>${table.name}</strong>
                <span>${active ? money(total) : "E lirë"}</span>
            </button>
        `;
    }).join("");
...
function renderMenu(category = "Të gjitha") {
    ...
    if (categories) {
        categories.innerHTML = allCategories.map(cat => `
            <button
...
function renderOrder() {
    ...
    if (!table || !table.items.length) {
        container.innerHTML =
            `<div class="empty-state">Nuk ka produkte në porosi.</div>`;
    ...
    container.innerHTML = table.items.map((item, index) => {
        const itemTotal =
        ...
        return `
            <div class="order-item">
                ...
                <div class="order-actions">
                    <button onclick="changeOrderQuantity(${index}, -1)">−</button>
                    <span>${item.quantity}</span>
                    <button onclick="changeOrderQuantity(${index}, 1)">+</button>
                    <button onclick="removeOrderItem(${index})">×</button>
                </div>
            </div>
        `;
    }).join("");
...
function openPayment() {
    ...
    const total = getOrderTotal(table);
...
function completePayment(method) {
    ...
    for (const item of table.items) {
        ...
        if (item.quantity > stock) {
            toast("Stoku nuk është i mjaftueshëm për " + item.name);
            return;
        }
    }
    ...
    const invoices = getJSON(INVOICES_KEY, []);
    ...
    const invoice = {
        ...
        waiterName: user ? user.name : "Pa emër",
        userName: user ? user.name : "Pa emër",
...
function showInvoice(invoice) {
    ...
    if (number) number.textContent = invoice.number;
    ...
    if (table) {
        table.textContent = invoice.tableName;
    }

    if (waiter) {
        waiter.textContent = invoice.waiterName;
    }

    if (items) {
        items.innerHTML = invoice.items.map(item => `
    ...
    if (total) {
        total.textContent = money(invoice.total);
    }
...
function updateDashboard() {
    ...
    if (invoiceToday) {
        invoiceToday.textContent = todayInvoices.length;
    }

    if (cardToday) {
        cardToday.textContent = money(card);
    }
    ...
    if (cashTotal) cashTotal.textContent = money(sales);
    if (cashMoney) cashMoney.textContent = money(cash);
    if (cashCard) cashCard.textContent = money(card);
    if (cashInvoices) cashInvoices.textContent = todayInvoices.length;
...
function renderPayments(invoices) {
    ...
    if (!invoices.length) {
        list.innerHTML =
            `<div class="empty-state">Nuk ka pagesa sot.</div>`;
        return;
    }
...
function renderHistory() {
    ...
    if (!invoices.length) {
        list.innerHTML =
            `<div class="empty-state">Nuk ka histori.</div>`;
        return;
    }
    ...
    if (usersList) {
        ...
        usersList.innerHTML = users.map((user, index) => `
            <div class="admin-row">
                ...
                ${
                    user.username !== "admin"
                        ? `<button onclick="deleteWaiter(${index})">Fshi</button>`
                        : ""
                }
            </div>
        `).join("");
...
function openWaiterModal() {
    ...
    const title =
        document.getElementById("generalModalTitle");

    if (title) {
        title.textContent = "Shto kamarier";
    }
}
...
function deleteWaiter(index) {
    ...
    if (users[index].username === "admin") return;
...
function toast(message) {
    ...
    if (!box) {
        ...
        box.style.color = "#fff";
        box.style.border = "1px solid #2d8f5b";
...
function setupSearch() {
    ...
    search.type = "search";
    search.placeholder = "Kërko produkt...";

    search.style.width = "100%";
    search.style.padding = "12px 14px";
    search.style.marginBottom = "15px";
    search.style.borderRadius = "12px";
    search.style.border = "1px solid #26352d";
    search.style.background = "#0d1410";
    search.style.color = "#fff";
    search.style.outline = "none";

    parent.insertBefore(search, menuGrid);
    ...
    search.addEventListener("input", () => {
        ...
        menuGrid.innerHTML = filtered.map(product => `
            <button
                class="menu-card"
                ${getStock(product.id) <= 0 ? "disabled" : ""}
                onclick="addProduct(${product.id})"
            >
                <strong>${escapeHTML(product.name)}</strong>
                <span>${money(product.price)}</span>
                <small>Stok: ${getStock(product.id)}</small>
            </button>
        `).join("");
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("MY BAR loaded successfully.");
});
...
window.deleteWaiter = deleteWaiter;
